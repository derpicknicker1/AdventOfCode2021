...
static char *open = "([{<" , *close = ")]}>", line[1000];
...
static bool getInput(char *f) {
	...
	while (fscanf(file, "%[^\n] ", line) != EOF) {
		bool run = true;
		int cnt = 0;
		for (int i = 0; i < strlen(line) && run; i++)
			if (strchr(open, line[i]))
				stack[cnt++] = strchr(open, line[i]) - open;
			else if (strchr(close, line[i]) - close != stack[--cnt]) {
				run = false;
				resultA += points[strchr(close, line[i]) - close];
			}

		if (run) {
			for (int i = cnt-1; i >= 0; i--)
				sums[cntS] = sums[cntS] * 5 + stack[i] + 1;
			cntS++;
		}
	}
