...
int solveB(int mode) {
	...
	while(++pos < len) {
		int most = 0, sf = len-pos-1;
		for (int i = 0; i < cnt; i++)
			if (!out[i])
				most += val[i] >> sf & 1 ? 1 : -1;

		int bit = most >= 0 ? mode : !mode;

		for (int i = 0; i < cnt; i++)
			if ((val[i] >> sf & 1) != bit && !out[i])
				out[i] = true;	
			else if(!out[i])
				found = i;
	}
...
void get3a(char * f) {
	...
	for (int i = 0; i < len; i++) {
		result1 += (arr[i] < 0 ? 1 : 0) << (len-i-1);
		result2 += (arr[i] > 0 ? 1 : 0) << (len-i-1);
	}
